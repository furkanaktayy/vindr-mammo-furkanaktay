...
def main():
    ...
    # Çıktı klasörü ve model ismi scratch olduğunu belirtecek şekilde güncellendi
    OUT_DIR = "./outputs_scratch"
    os.makedirs(OUT_DIR, exist_ok=True)
    MODEL_PATH = os.path.join(OUT_DIR, "resnet50_density_from_scratch.pth")
    
    IMG_SIZE = (512, 512)
    BATCH_SIZE = 8 
    EPOCHS = 25  # Sıfırdan eğitim için epoch sayısı genellikle daha yüksek tutulmalıdır
    LR = 1e-4
    NUM_CLASSES = 4
    ...
    print("\n" + "="*40)
    print("Data Loaded Successfully (SCRATCH MODE):")
    ...
    # --- Model Definition (Pretrained=False) ---
    # ResNet50 mimarisi sıfırdan oluşturulur
    model = models.resnet50(weights=None) # weights=None sıfırdan eğitim sağlar
    ...
    # --- Training ---
    print(f"Starting SCRATCH Training Phase on {DEVICE}...")
    best_val_auc = 0.0
    ...
    for epoch in range(EPOCHS):
        ...
        # Olasılık toplamlarının tam 1.0 olmasını garanti etmek için yüksek hassasiyetli normalizasyon
    ...
    print("\n" + "="*45)
    print("FINAL TEST RESULTS (SCRATCH MODEL)")
    print("-" * 45)
    ...
    plt.title("Confusion Matrix (Scratch ResNet50)")
